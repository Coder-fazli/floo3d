...
const Tooltip: React.FC<TooltipProps> = ({ text, children, direction }) => {
  ...
  return (
    <div onMouseEnter={() => setVisible(true)} onMouseLeave={() => setVisible(false)} className="relative inline-block">
      {children}
      {visible && (
        <div
          className={`${
...
export default function Speeddial({ direction, actionButtons }: SpeedialProps) {
  ...
  const getAnimation = () => {
    switch (direction) {
      case "up":    return "origin-bottom flex-col order-0";
      case "down":  return "origin-top flex-col order-2";
      case "left":  return "origin-right order-0";
      case "right": return "origin-left order-2";
      default:      return "";
    }
  };

  const getGlassyClasses = () =>
    "backdrop-filter backdrop-blur-xl bg-white border border-white rounded-xl shadow-lg transition duration-300";
  ...
  return (
    ...
    >
      <button
        ...
        className={`${getGlassyClasses()} order-1 flex items-center gap-2 px-4 py-3 text-gray-800 transition duration-300 hover:bg-slate-100`}
      >
        <Plus size={20} />
        <span className="text-sm font-semibold">Generated Renders</span>
      </button>
